...
class StudentTable extends Table
{
    ...
    public function validationDefault(Validator $validator): Validator
    {
        ...
        $validator
            ->scalar('s_name')
            ->maxLength('s_name', 150)
            ->requirePresence('s_name', 'create')
            ->notEmptyString('s_name');

        $validator
            ->date('s_dob')
            ->requirePresence('s_dob', 'create')
            ->notEmptyDate('s_dob');

        $validator
            ->integer('s_class')
            ->requirePresence('s_class', 'create')
            ->notEmptyString('s_class');

        $validator
            ->scalar('s_gender')
            ->maxLength('s_gender', 10)
            ->requirePresence('s_gender', 'create')
            ->notEmptyString('s_gender');
